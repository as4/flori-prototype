...
- **iOS physical silent switch mutes WebAudio** (unlike HTML5 `<audio>`). `navigator.audioSession.type = 'playback'` (Safari 17.2+) opts into music-app semantics and overrides the switch — but on recent iOS Safari builds this alone isn't enough: the audio session needs an *actively playing source* to hold the playback category. We keep a silent looping `AudioBufferSourceNode` running through the master gain for the lifetime of the page (started inside the first PTT gesture). HTMLAudio loops work too, but produce a more prominent Safari-branded Now Playing card on the lock screen; AudioContext-only gives an empty card.
- **iOS lock-screen Now Playing card is unavoidable when overriding the silent switch.** Any active "playback" audio session triggers the card. We minimize it by (a) using AudioContext-only audio (no HTMLAudio loop), and (b) setting `navigator.mediaSession.metadata = null` + `playbackState = 'none'` in `ensureAudioReady`. Result: an empty card with no track info — but it doesn't disappear. Removing the silent loop kills the card *and* the silent-switch override; they're tied together by iOS design.
